...
const variantClasses = {
  primary: "bg-gradient-to-b from-gold to-gold-dark text-black border border-white shadow-[0_2px_4px_rgba(0,0,0,0.5)] hover:enabled:from-gold-light hover:enabled:to-gold hover:enabled:shadow-[0_0_10px_rgba(212,175,55,0.6)]",
...
export const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button 
      ...
      data-testid={`btn-${typeof children === 'string' ? children.toLowerCase().replace(/\s+/g, '-') : 'custom'}`}
      {...props}
    >
      {children}
    </button>

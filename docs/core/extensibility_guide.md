...
在之前的版本中，我们在 `src/simulate.ts` 中直接硬编码了卡牌和赞助者的数据。现在这些数据已经被提取到了独立的文件 `src/data/initialData.ts` 中（包含 `TIER_1_CARDS`, `TIER_2_CARDS`, `TIER_3_CARDS`, `ALL_PATRONS`）。

尽管数据已经分离到了 `data` 目录中，但理想的“配置驱动”仍然建议在后续的生产版本迭代中，将这些数据进一步提取为纯 JSON 文件（例如 `src/data/cards.json` 和 `src/data/patrons.json`），并实现对应的数据加载逻辑，以确保系统的最大可扩展性和可维护性。
